...
class TableHandler {
    ...
    auto getTypeDefinition(std::string &type, std::string &name) {
        ...
        if (type.compare("integer") == 0) {
            ptr = std::move(std::unique_ptr<AbstractAttribute>(
                    reinterpret_cast<AbstractAttribute *>(new TableAttribute<long>())));
        } else if (type.compare("string") == 0) {
            ptr = std::move(std::unique_ptr<AbstractAttribute>(
                    reinterpret_cast<AbstractAttribute *>(new TableAttribute<std::string>())));
        } else if (type.compare("binary") == 0) {
            ptr = std::move(std::unique_ptr<AbstractAttribute>(
                    reinterpret_cast<AbstractAttribute *>(new TableAttribute<char *>())));
        } else if (type.compare("real") == 0) {
            ptr = std::move(std::unique_ptr<AbstractAttribute>(
                    reinterpret_cast<AbstractAttribute *>(new TableAttribute<double>())));
        } else {
            throw 1;
        }
